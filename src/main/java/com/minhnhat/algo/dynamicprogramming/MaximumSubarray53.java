...
public class MaximumSubarray53 {
  ...
  public static int action(int[] nums, Map<String, Integer> m) {
    if (nums.length == 0) {
      return 0;
    }
    if (nums.length == 1) {
    ...
    if (nums.length == 2) {
      if (m.get(arrayToString(nums)) != null) {
        System.out.println("find for " + arrayToString(nums) + "=>" + m.get(arrayToString(nums)));
        return m.get(arrayToString(nums));
      } else {
        System.out.println("dont find max for " + arrayToString(nums));
        int max = max(sum(nums), max(nums[0], nums[1]));
        m.put(arrayToString(nums), max);
        return max;
      }
    }
    for (int i = 0; i < nums.length; i++) {
      int[] subs = subArray(nums, 0, i);
      // TODO
      int maxSub = action(subs, m);
      m.put(arrayToString(subs), maxSub);
    }
    return m.get(arrayToString(nums));
  }
