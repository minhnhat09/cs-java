...
public class LambdaCollection {
  public static void main(String[] args) {
    ...
    TreeSet<String> treeSetNames = people.stream().map(p -> p.getName()).collect(Collectors.toCollection(() -> new TreeSet<>()));
    System.out.println(treeSetNames);

    // String to map count char
    String alpha = "abeddddfq";
    Map<Character, Long> collect = alpha.chars().mapToObj(c -> (char) c).collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    System.out.println(collect);

    // Divide list by 2 (true and false)
    Map<Boolean, List<Person>> supperThan25 = people.stream().collect(Collectors.partitioningBy(person -> person.getAge() > 25));
    ...
    Map<Integer, Long> countAge = people.stream().collect(Collectors.groupingBy(Person::getAge, Collectors.counting()));
    System.out.println(countAge);
    // Grouping and maping

    // people.stream().collect(Collectors.groupingBy(Person::getAge, Collectors.mapping(person->person.getName())));
